...
int main(){
	...
	cout<<"\nNow we are inserting a node after the node pointed by p\n";
	
	struct node *l;
	l = new node();
	cout<<"Enter data in new node which will be inserted after the node pointed by p\n";
	cin>>l->data;
	
	l->next = p->next;
	p->next = l;
	cout<<"We have added a node after a node pointed by p\n";
