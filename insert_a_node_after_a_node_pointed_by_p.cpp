...
int main(){
	...
	int response2;
	cout<<"Enter 1 if you want to insert a node after the node pointed by p in a circular linked list: ";
	cin>>response2;
	if(response2 == 1){
		struct node *t, *l;
		t = new node();
		cout<<"Enter data in the new node which will be inserted after the node pointed by p: ";
		cin>>t->data;
		l = p->next;
		p->next = t;
		t->next = l;		
	...
	cout<<"New Node inserted after the node pointed by p in a circular linked list\n";
