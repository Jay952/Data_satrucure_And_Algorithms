...
int main(){
	...
	int response2;
	cout<<"Enter 1 if you want to add a dnode after the dnode pointed by p in a doubly linked list\n";
	cin>>response2;
	if(response == 1){
		struct dnode *t, *l;
		t = new dnode();
	...
	struct dnode *u;
	u = head;
	cout<<"Data of all dnodes after inserting a new dnode after the dnode pointed by p are respectively: ";
	while(u != NULL){
		cout<<u->data<<" ";
		u = u->next;
	}
